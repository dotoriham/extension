export const SERVER_URL = 'https://dotoriham.duckdns.org';
export const MAIN_URL = 'https://dotoriham.com';

// assets path
export const HOME_ICON = 'https://i.ibb.co/zmc2Vcy/Home-24.png';
export const BELL_SELECTED_ICON = 'https://i.ibb.co/XSrKhLZ/Bell-selected.png';
export const BELL_UNSELECTED_ICON =
  'https://i.ibb.co/X4VYcPH/Bell-unselected.png';
export const FOLDER_ICON =
  'https://images.velog.io/images/ksmfou98/post/bfced404-c12b-4fc7-98d0-05ce7a36f2de/Folder_16.png';
export const ARROW_DOWN_ICON =
  'https://images.velog.io/images/ksmfou98/post/47ee02fd-78f1-43b6-81d9-881d6e3b3ffa/Arrow_down_s.png';
export const ARROW_SIDE_ICON =
  'https://images.velog.io/images/ksmfou98/post/1d15c5b9-70eb-4b16-b20f-b15c8f602cf1/Arrow_side_s.png';
export const SYMBOL_ICON = 'https://i.ibb.co/XfBmgYb/Group-1698.png';
export const DEFAULT_ICON = 'https://i.ibb.co/806SKzh/Symbol-36.png';
export const PLUS_ICON = 'https://i.ibb.co/PhFrPHf/Plus.png';
export const LOADING_ICON = 'https://i.ibb.co/0VvhYSc/loading.webp';
